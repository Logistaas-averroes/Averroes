...
-- Stable dedupe key: attribution_key (SHA1 of gclid|contact_id|deal_id|campaign_name|keyword|match_status)
-- Multiple deals for the same contact/GCLID are preserved as separate rows.
CREATE TABLE IF NOT EXISTS gclid_attribution (
  id                  SERIAL PRIMARY KEY,

  -- Stable dedupe key generated by the writer (SHA1 of key fields).
