...
def pull_closed_won_deals_with_sources_in_range(date_from: str, date_to: str) -> list:
    ...
    Returns [{deal_id, deal_close_date, deal_amount_usd,
...
def _fetch_associated_contact_ids(deal_id) -> list:
    ...
    try:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {HUBSPOT_API_KEY}"}, timeout=30)
        resp.raise_for_status()
        ids = []
        ...
        logger.warning("Failed to fetch contacts for deal %s: %s", deal_id, exc)
        return []
...
def _fetch_contact_source_props(contact_id) -> dict:
    ...
    try:
        ...
        logger.warning("Failed to read source for contact %s: %s", contact_id, exc)
        return {}
